...
function SectionDivider({ label, count }: { label: string; count: number }) {
  return (
    <tr className="bg-slate-950/80">
      <td colSpan={8} className="px-4 py-2 text-xs font-semibold uppercase tracking-widest text-slate-500 border-t border-slate-800">
...
export function LeadTable({ leads, showCaller, onStatusChange, onNotesChange }: Props) {
  ...
  const noWebsite  = leads.filter(l => !l.website?.trim())
  const hasWebsite = leads.filter(l =>  l.website?.trim())
