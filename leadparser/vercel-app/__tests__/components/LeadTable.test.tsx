...
describe('LeadTable — score badge', () => {
  it('renders the lead score inside the badge', () => {
    render(<LeadTable leads={[makeLead({ lead_score: 18 })]} />)
    // ScoreBadge renders "{score} HOT/WARM/…" as a single span — use regex
...
describe('LeadTable — status display', () => {
  ...
  it('calls onStatusChange when status select changes', async () => {
    ...
    render(
      <LeadTable
        leads={[makeLead({ id: 'lead-1', status: 'new' })]}
...
describe('LeadTable — expand/collapse', () => {
  ...
  it('shows detail row with address after clicking Expand', () => {
    render(<LeadTable leads={[makeLead({ address: '123 Main St' })]} />)
    fireEvent.click(screen.getByText('Expand'))
    expect(screen.getByText(/123 Main St/)).toBeInTheDocument()
  })

  it('shows pitch notes in expanded row', () => {
    render(<LeadTable leads={[makeLead({ pitch_notes: 'Unique pitch note XYZ' })]} />)
    fireEvent.click(screen.getByText('Expand'))
    expect(screen.getByText('Unique pitch note XYZ')).toBeInTheDocument()
  })

  it('collapses the detail row after clicking Hide', () => {
    render(<LeadTable leads={[makeLead({ address: '999 Unique Ln' })]} />)
    fireEvent.click(screen.getByText('Expand'))
    fireEvent.click(screen.getByText('Hide'))
    expect(screen.queryByText(/999 Unique Ln/)).not.toBeInTheDocument()
  })

  it('shows notes textarea when onNotesChange is provided and row is expanded', () => {
    const handler = vi.fn().mockResolvedValue(undefined)
